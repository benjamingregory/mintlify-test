...
You can also reference existing files rather than pasting code for existing code.
```
Review the EnemyMovement.cs script and make them less predictable.
Something is wrong with the CameraController; take a look and make sure it follows the player.
```
